...
const isMIAMVersion = (x: number): x is MIAMVersion =>
  Object.values(MIAMVersion).includes(x as MIAMVersion);
const isMIAMFid = (x: string): x is MIAMFid =>
  Object.values(MIAMFid).includes(x as MIAMFid);
const isMIAMCoreApp = (x: number): x is MIAMCoreApp =>
  Object.values(MIAMCoreApp).includes(x as MIAMCoreApp);
const isMIAMCorePdu = (x: number): x is MIAMCorePdu =>
  Object.values(MIAMCorePdu).includes(x as MIAMCorePdu);
...
export class MIAMCoreUtils {
  static AppTypeToAppIdLenTable: Record<
    MIAMVersion,
    Record<MIAMCoreApp, number>
  > = {
    [MIAMVersion.V1]: {
  ...
  static FidHandlerTable: Record<MIAMFid, (txt: string) => PduDecodingResult> =
    {
      [MIAMFid.SingleTransfer]: (txt: string) => {
        ...
        if (isMIAMVersion(version) && isMIAMCorePdu(pduType)) {
          const versionPduHandler =
            this.VersionPduHandlerTable[version][pduType];
          return versionPduHandler(hdr, body);
        } else {
          return {
            decoded: false,
            error:
              'Invalid version and PDU type combination: v=' +
              version +
              ', pdu=' +
              pduType,
          };
      ...
      [MIAMFid.FileTransferRequest]: () => {
        return {
          decoded: false,
          error: 'File Transfer Request not implemented',
        };
      },
      [MIAMFid.FileTransferAccept]: () => {
        return {
          decoded: false,
          error: 'File Transfer Accept not implemented',
        };
      },
      [MIAMFid.FileSegment]: () => {
        return { decoded: false, error: 'File Segment not implemented' };
      },
      [MIAMFid.FileTransferAbort]: () => {
        return { decoded: false, error: 'File Transfer Abort not implemented' };
      },
      [MIAMFid.XOFFIndication]: () => {
        return { decoded: false, error: 'XOFF Indication not implemented' };
      },
      [MIAMFid.XONIndication]: () => {
  ...
  private static corePduDataHandler(
    version: MIAMVersion,
    minHdrSize: number,
    crcLen: number,
    hdr: Buffer,
    ...
    if (hdr.length < minHdrSize) {
      return {
        ...
        error:
          'v' +
          version +
          ' header size too short; expected >= ' +
          minHdrSize +
          ', got ' +
          hdr.length,
      };
    ...
    if (version === MIAMVersion.V1) {
      ...
      if (pduSize > msgSize) {
        pduIsComplete = false;
        pduErrors.push(
          'v1 PDU truncated: expecting ' + pduSize + ', got ' + msgSize,
        );
      }
      hdr = hdr.subarray(4);
    ...
    const pduIsACARS =
      [
        MIAMCoreApp.ACARS2Char,
        MIAMCoreApp.ACARS4Char,
        MIAMCoreApp.ACARS6Char,
    ...
    if (body !== undefined && body.length > 0) {
      if (
        [MIAMCoreV1Compression.Deflate, MIAMCoreV2Compression.Deflate].indexOf(
          pduCompression,
        ) >= 0
      ) {
        try {
        ...
        [MIAMCoreV1Compression.None, MIAMCoreV2Compression.None].indexOf(
          pduCompression,
        ) >= 0
      ) {
        pduData = body;
        ...
        pduErrors.push(
          'Unsupported v' + version + ' compression type: ' + pduCompression,
        );
      }
      ...
      if (pduData !== null) {
        ...
        if (crcCheck === pduCrc) {
          ...
          pduErrors.push(
            'Body failed CRC check: provided=' +
              pduCrc +
              ', generated=' +
              crcCheck,
          );
