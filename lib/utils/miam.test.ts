import {
  MIAMCoreUtils,
  MIAMCoreV1Compression,
  MIAMCoreV2Compression,
} from './miam';

test('v1, compressed, acars', () => {
  const msg =
    "T32!<<,W/jVEJ5u(@\\!'s.16q/)<:-JXX|Q&a)r_CuOS?R65eRb:E8<DV/R('OXUd&Vsp5njIZhc3K&[\"hCafn>bHPILl$J-V*::RL,d]?So_4M_VS;Ln\"U,;8u?`,7j=ACsP@,t.rGoaFFQm1S7*'>kRs>,:u>X?oe2->Z-5`_Ztu,Fr.R(jR7>J^s-94:^OqFYrF][5q?tMocL[p%^T#7).P:W;.$4Ym1#&8iu%ac;%S_9i<e!Lp`bDFhu;eR!R&T>qkLZ_rC^NA6gllCR_sE-?$k^?N:'+";
  ...
  expect(decodeResult.message.data.compression).toBe(
    MIAMCoreV1Compression.Deflate,
  );
...
test('v1, compressed, acars, incomplete', () => {
  const msg =
    "T12!<<B[67joO3AE3:!'s.16q2Tb:9FQs|X]eG[j2M]0.rudB<$2(Xj>:Whn0'KEp#_@kN+<G'I.G-DIX^RW,uJ,M,&9<XO?406A(Bnse^_UPF+su$OC:KOI/*p=F8Gh5:Or%)B`e/.%tN-jYGidD%+[OhHG_Wc9K^E3Skpit$/,N);FlMj%g)Orhs^\"es8)BPa6C51Im?a^0@S64/";
  const decodeResult = MIAMCoreUtils.parse(msg);
  ...
  expect(decodeResult.message.data.acars.mfi).toBe(undefined);
  expect(decodeResult.message.data.acars.text).toBe(
    'A350,000130,1,1,TB000000/REP019,24,02;H01,019,24,02,6400,00175,B-18910,3,0,03,11,19,08,07,01,070/H02,NZAA YBBN,CAL054  ,S',
  );
});
