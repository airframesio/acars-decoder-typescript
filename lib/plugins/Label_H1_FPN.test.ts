...
describe('Label_H1 FPN', () => {
  ...
  test('decodes Label H1 Preamble FPN full flight', () => {
    ...
    expect(decodeResult.formatted.items[7].value).toBe(
      'EAGUL6 starting at ZUN',
    );
    expect(decodeResult.formatted.items[8].label).toBe('Approach Procedure');
    expect(decodeResult.formatted.items[8].value).toBe(
      'ILS26: >> AIR(40.017 N, 80.817 W) > J110 > BOWRR >> VLA(39.093 N, 89.162 W) >> STL(38.860 N, 90.482 W) >> GIBSN(38.717 N, 92.407 W) >> TYGER(38.683 N, 94.083 W) >> GCK(37.918 N, 100.725 W) >> DIXAN(36.282 N, 105.955 W) >> ZUN(34.965 N, 109.155 W)',
    );
  ...
  test('decodes Label H1 Preamble FPN in-flight', () => {
    ...
    expect(decodeResult.formatted.items[3].value).toBe(
      'KAYEX(36.487 N, 120.948 W) >> LOSHN(35.848 N, 120.000 W) >> BOILE(34.422 N, 118.027 W) >> BLH(33.597 N, 114.762 W)',
    );
  ...
  test('decodes Label H1 Preamble FPN with WS', () => {
    ...
    expect(decodeResult.formatted.items[2].value).toBe(
      'KMSP..N55064W000477..N55163W001141..ERAKA..N60000W020000..N61000W030000',
    ); // FIXME - just 'KMSP'
  ...
  test('decodes Label H1 Preamble FPN with newlines', () => {
    ...
    expect(decodeResult.formatted.items[3].value).toBe(
      'YMML..MEMUP,S33451E120525.Y53.WENDY',
    ); //TODO - pull out route
    expect(decodeResult.formatted.items[4].label).toBe('Message Checksum');
  ...
  test('decodes Label H1 Preamble FPN with SN and TS', () => {
    // https://app.airframes.io/messages/2372685289
    message.text =
      'FPN/TS155631,170224/SN155631/RP:DA:PHNL:AA:KASE:D:MKK5.KOLEA:F:KOLEA,N22354W155133..CLUTS,N23002W154393.R465.CINNY,N36109W124456..OAL,N38002W117462.J58.ILC,N38150W114237..EYELO,N38455W110469..SAKES,N38500W110163.J80.DBL,N39264W106537F5E1';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[3].value).toBe(
      'MKK5 starting at KOLEA',
    );
    expect(decodeResult.formatted.items[4].label).toBe('Aircraft Route');
    expect(decodeResult.formatted.items[4].value).toBe(
      'KOLEA(22.590 N, 155.222 W) >> CLUTS(23.003 N, 154.655 W) > R465 > CINNY(36.182 N, 124.760 W) >> OAL(38.003 N, 117.770 W) > J58 > ILC(38.250 N, 114.395 W) >> EYELO(38.758 N, 110.782 W) >> SAKES(38.833 N, 110.272 W) > J80 > DBL(39.440 N, 106.895 W)',
    );
  ...
  test('decodes Label H1 Preamble FPN with FN', () => {
    ...
    message.text =
      'FPN/RP:DA:KSEA:AA:KPHX:D:SUMMA2.SUMMA:F:SUMMA,N46371W121593..LTJ,N45428W121061..IMB,N44389W119427.Q35..CORKR,N36050W112240..TENTS,N35295W112271:A:BRUSR1.TENTS/FNFFT17245D16';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[3].label).toBe('Departure Procedure');
    expect(decodeResult.formatted.items[3].value).toBe(
      'SUMMA2 starting at SUMMA',
    );
    expect(decodeResult.formatted.items[4].label).toBe('Aircraft Route');
    expect(decodeResult.formatted.items[4].value).toBe(
      'SUMMA(46.618 N, 121.988 W) >> LTJ(45.713 N, 121.102 W) >> IMB(44.648 N, 119.712 W) > Q35 >> CORKR(36.083 N, 112.400 W) >> TENTS(35.492 N, 112.452 W)',
    );
    expect(decodeResult.formatted.items[5].label).toBe('Arrival Procedure');
    expect(decodeResult.formatted.items[5].value).toBe(
      'BRUSR1 starting at TENTS',
    );
  ...
  test('decodes Label H1 Preamble #M1BFPN', () => {
    ...
    message.text =
      '#M1BFPN/RI:DA:KJFK:AA:KPHX:CR:JFKPHX20..KG701..KG702..KP702..KP703..KD601..KD602..PUB..ALS.J102.GUP:A:EAGUL6.GUP:F:HOMRR90E1';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[3].label).toBe('Company Route');
    expect(decodeResult.formatted.items[3].value).toBe(
      'JFKPHX20: >> KG701 >> KG702 >> KP702 >> KP703 >> KD601 >> KD602 >> PUB >> ALS > J102 > GUP',
    );
    expect(decodeResult.formatted.items[4].label).toBe('Arrival Procedure');
    expect(decodeResult.formatted.items[4].value).toBe(
      'EAGUL6 starting at GUP',
    );
    expect(decodeResult.formatted.items[5].label).toBe('Aircraft Route');
  ...
  test('decodes Label H1 #M1BFPN No Preamble', () => {
    ...
    message.text =
      'F37AKL0767#M1BFPN/TS232008,022324/RP:DA:TNCA:AA:TNCB:R:11O:D:ADRI1F..IRLEP.A574..PJG:AP:RNV10(10O)/PR,,110,,183,7,13,,M7,25,,,P30,M40,36090,13,3455,300/DTTNCB,10O,119,23440847C0';
    const decodeResult = plugin.decode(message);
  ...
  test('decodes Label H1 Preamble FPN <invalid>', () => {
    message.text = 'FPN Bogus message';
    const decodeResult = plugin.decode(message);
  ...
  test('decode RM', () => {
    // TODO: enable
    message.text =
      'FPN/ID88194A,RCH857,PMZM107QP021/MR2,/RM:AA:FJDG:F:DOH.N300..NOLSU.P307..SETSI.P307..PARAR..N20000E063000..RIGLO.L516..ELKEL.L516..BUMMR/WP,,,,E3E9';
    const decodeResult = plugin.decode(message);
