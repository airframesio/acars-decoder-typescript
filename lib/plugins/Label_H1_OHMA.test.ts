...
describe('Label H1 Preamble OHMA', () => {
  ...
  test('decodes Label H1 Preamble OHMA RTN', () => {
    ...
    message.text =
      '/RTNBOCR.OHMAeJyrVipLLSrOzM9TslIy0jNQ0lHKTS0uTkxPBfJL81JS0zLzUlOUagH7TQzW';
  ...
  test.skip('decodes Label H1 Preamble OHMA partial', () => {
    ...
    message.text =
      '#T1B/RTNBOCR.OHMAeJy1km+vmjAUxr/K0tfSlPLXvuMi5pJRL0Hc3MZCOu0cSUFTqsly43dfBb3zKmavBq9O+3uec84Dr+DAZVttG0AAhgiMQM3blm24rl8LsBIVb1S8LgApwMszDQowKi7IhCneXWCEbQP5BsI58gmyiWVD3/S+dvCaKaYpbcYquROs4a0uv+lasUrM9vUPLjuXZ';
    const decodeResult = plugin.decode(message, { debug: true });
    ...
    expect(decodeResult.message).toBe(message);
    expect(decodeResult.raw.ohma).toBe(
      '{"version":"2.0","message":"{"clientId":"OHMA","messageDate":"2024-08-02T08:04:34.817Z","data":{"airplanes":[{"tailNumber":"',
    );
    expect(decodeResult.formatted.items.length).toBe(1);
    expect(decodeResult.formatted.items[0].type).toBe('ohma');
    expect(decodeResult.formatted.items[0].code).toBe('OHMA');
  ...
  test('decodes Label H1 Preamble OHMA partial', () => {
    ...
    message.text =
      'OHMAeJy1lF+PmkAUxb9Kw/MyGRj+v1HEYhaQ4OxqrI2Z6tSSIBpAk2bjd+8Aau3szCZ9qD4NnN899x7v+Kacad0Uh0rxFB1A5UnZ06YhO8rObytlUxa0aifbleKtlGmU+CvlaXWTjEhL+xc61A0Vuiq0MHQ90/UMBJBjL3vxlrSEqVgxUtTHklS0Ycev7NySokxP+++07qvMMj';
    const decodeResult = plugin.decode(message, { debug: true });
    ...
    expect(decodeResult.message).toBe(message);
    expect(decodeResult.raw.ohma).toBe(
      '{"version":"2.0","message":"{\\"clientId\\":\\"OHMA\\",\\"messageDate\\":\\"2024-09-06T09:59:43.387Z\\",\\"data\\":{\\"airplanes\\":[{\\"tailNumber\\":\\"SP',
    );
    expect(decodeResult.formatted.items.length).toBe(1);
    expect(decodeResult.formatted.items[0].type).toBe('ohma');
    expect(decodeResult.formatted.items[0].code).toBe('OHMA');
