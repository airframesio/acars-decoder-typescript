    ...
    if (results?.groups) {
      ...
      ResultFormatter.position(
        ...
        CoordinateUtils.decodeStringCoordinatesDecimalMinutes(
          results.groups.unsplit_coords,
        ),
      );
      const flight_level =
        results.groups.flight_level_or_ground == 'GRD' ||
        results.groups.flight_level_or_ground == '***'
          ? 0
          : Number(results.groups.flight_level_or_ground);

      ResultFormatter.month(
        decodeResult,
        Number(results.groups.current_date.substring(0, 2)),
      );
      ResultFormatter.day(
        decodeResult,
      ...
      ResultFormatter.eta(
        decodeResult,
        DateTimeUtils.convertHHMMSSToTod(results.groups.eta_time + '00'),
      );
