...
export class Label_H1_Slash extends DecoderPlugin {
  ...
  decode(message: Message, options: Options = {}): DecodeResult {
    ...
    if (
      headerData[1] === 'POS' &&
      fields[2].startsWith('TS') &&
      fields[2].length > 15
    ) {
      // variant 3 hack
      // rip out the timestamp and process the rest
      H1Helper.processPosition(
        decodeResult,
        fields[2].substring(15).split(','),
      );
      ...
      H1Helper.processPosition(
        decodeResult,
        headerData[1].substring(3).split(','),
      );
    } else {
    ...
    for (let i = 2; i < fields.length; i++) {
      ...
      if (field.startsWith('TS')) {
        H1Helper.processTimeStamp(
          decodeResult,
          field.substring(2, 15).split(','),
        );
