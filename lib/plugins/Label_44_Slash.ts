  ...
  decode(message: Message, options: Options = {}): DecodeResult {
    ...
    if (data.length >= 6) {
      ...
      ResultFormatter.position(decodeResult, {
        latitude: ((data[0].charAt(0) === 'S') ? -1 : 1) * parseFloat(data[0].slice(1).trim()),
        longitude: ((data[1].charAt(0) === 'W') ? -1 : 1) * parseFloat(data[1].slice(1).trim())});
      ...
      ResultFormatter.eta(decodeResult, DateTimeUtils.convertHHMMSSToTod(data[5]));
      if (data.length === 18) {
