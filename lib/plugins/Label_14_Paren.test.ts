...
import { Label_14_Paren } from './Label_14_Paren';

describe('Label_14_Paren', () => {
    let plugin: Label_14_Paren;

    beforeEach(() => {
        const decoder = new MessageDecoder();
        plugin = new Label_14_Paren(decoder);
    });

    test('matches Label 10 Preamble paren qualifiers', () => {
        expect(plugin.decode).toBeDefined();
        expect(plugin.name).toBe('label-10-paren');
      ...
      test('decodes Label 10 Preamble (2 partial', () => {
    ...
    test('decodes Label 10 Preamble (2 complete', () => {
    ...
    test('decodes Label 10 Preamble / <invalid>', () => {
        ...
        expect(decodeResult.decoder.name).toBe('label-10-paren');
