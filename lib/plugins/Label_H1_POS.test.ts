...
describe('Label_H1 POS', () => {
  ...
  test('variant 1', () => {
    ...
    message.text = 'POSN43312W123174,EASON,215754,370,EBINY,220601,ELENN,M48,02216,185/TS215754,0921227A40';
  ...
  test('variant 2', () => {
    ...
    message.text = 'POSN45209W122550,PEGTY,220309,134,MINNE,220424,HISKU,M6,060013,269,366,355K,292K,730A5B';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 3', () => {
    ...
    message.text = 'POSN43030W122406,IBALL,220516,380,AARON,220816,MOXEE,M47,0047,86/TS220516,092122BF64';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 4', () => {
    ...
    message.text = 'POSN33225W079428,SCOOB,232933,340,ENEME,235712,FETAL,M42,003051,15857F6';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 5', () => {
    ...
    message.text = 'POSN38531W078000,CSN-01,112309,310,CYN-02,114151,ACK,M40,26067,22479226';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 6', () => {
    ...
    message.text = 'POS/RFSCOOB.KEMPR.ECG.OHPEA.TOMMZ.OXANA.ZZTOP.OMALA.WILYY.KANUX.GALVN.KASAR.LNHOM.SLUKA.FIPEK.PUYYA.PLING.KOLAO.JETSSF2FC';
    const decodeResult = plugin.decode(message);
  ...
  test('#short variant', () => {
    ...
    message.text = '#M1BPOSN37533W096476,ROKNE,185212,330,DOSOA,190059,BUM,M50,272100,1571541'
    const decodeResult = plugin.decode(message);
  ...
  test.skip('# long variant', () => {
    ...
    message.text = '#M1BPOSN29510W098448,RW04,140407,188,TATAR,4,140445,ALISS,M12,246048,374K,282K,1223,133,KSAT,KELP,,70,151437,415,73/PR1223,222,240,133,,44,40,252074,M22,180,P0,P0/RI:DA:KSAT:AA:KELP..TATAR:D:ALISS6:F:ALISS..FST';
    const decodeResult = plugin.decode(message);
  ...
  test('# variant 7', () => {
    ...
    message.text = 'F37AMCLL93#M1BPOS/ID746026,,/DC03032024,173207/MR1,/ET031846/PSN42579W108090,173207,320,WAIDE,031759,WEDAK,M49,267070,T468/CG264,110,360/FB742/VR324E17';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 8', () => {
    ...
    message.text = 'POS/TS080616,210324/DTMMGL,29O,64,103316/PR1754,231,350,189,,0,0,,M45,185,,,P16,P0,36000,,1565,250/RP:DA:MMTJ:AA:MMGL:R:27O:D:TUMA2B..SANFE.UT4..LMM:A:LONV1D:AP:ILSZ29.PLADE(29O)9D1C';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 1 with offset', () => {
    ...
    // https://app.airframes.io/messages/2500335076
    message.text = 'POSN33204W114082,BLH176-0093,062056,330,SALOM180-0127,062211,KOFFA180-0148,M49,30628,3201251';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 9', () => {
    ...
    message.text = 'POSN39164W077259,FORKL,231828,51,THRET,231917,TOOPR,P16,1726,167,/TS231828,2807244841';
    const decodeResult = plugin.decode(message);
  ...
  test('decodes duplicate data', () => {
    message.text = 'POSN39220W078258,MRB18,044034,340,EYT19,044427,COL20,M49,27369,436,813/PSN39220W078258,MRB18,044034,340,EYT19,044427,COL20,M49,27369,436,813,ECON CRZ,0,25140035'
    const decodeResult = plugin.decode(message);
