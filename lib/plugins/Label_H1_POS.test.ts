...
describe('Label_H1 POS', () => {
  ...
  test('variant 1', () => {
    ...
    // Raw field checks
    expect(decodeResult.raw.position.latitude).toBeCloseTo(43.52, 3);
    expect(decodeResult.raw.position.longitude).toBeCloseTo(-123.29, 3);
  ...
  test('variant 2', () => {
    ...
    expect(decodeResult.formatted.items.length).toBe(5);
    // Raw field checks
    expect(decodeResult.raw.position.latitude).toBeCloseTo(45.348, 2);
    expect(decodeResult.raw.position.longitude).toBeCloseTo(-122.917, 2);
  ...
  test('variant 3', () => {
    ...
    expect(decodeResult.formatted.items.length).toBe(5);
    // Raw field checks
    expect(decodeResult.raw.position.latitude).toBeCloseTo(43.05, 2);
    expect(decodeResult.raw.position.longitude).toBeCloseTo(-122.677, 2);
  ...
  test('variant 4', () => {
    ...
    expect(decodeResult.formatted.items.length).toBe(5);
    // Raw field checks
    expect(decodeResult.raw.position.latitude).toBeCloseTo(33.375, 2);
    expect(decodeResult.raw.position.longitude).toBeCloseTo(-79.713, 2);
  ...
  test('variant 5', () => {
    ...
    message.text =
      'POSN38531W078000,CSN-01,112309,310,CYN-02,114151,ACK,M40,26067,22479226';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items.length).toBe(5);
    // Raw field checks
    expect(decodeResult.raw.position.latitude).toBeCloseTo(38.885, 2);
    expect(decodeResult.raw.position.longitude).toBeCloseTo(-78.0, 2);
  ...
  test('variant 6', () => {
    ...
    message.text =
      'POS/RFSCOOB.KEMPR.ECG.OHPEA.TOMMZ.OXANA.ZZTOP.OMALA.WILYY.KANUX.GALVN.KASAR.LNHOM.SLUKA.FIPEK.PUYYA.PLING.KOLAO.JETSSF2FC';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[1].value).toBe(
      'SCOOB > KEMPR > ECG > OHPEA > TOMMZ > OXANA > ZZTOP > OMALA > WILYY > KANUX > GALVN > KASAR > LNHOM > SLUKA > FIPEK > PUYYA > PLING > KOLAO > JETSS',
    );
  ...
  test('#short variant', () => {
    ...
    message.text =
      '#M1BPOSN37533W096476,ROKNE,185212,330,DOSOA,190059,BUM,M50,272100,1571541';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[2].value).toBe(
      'ROKNE@18:52:12 > DOSOA@19:00:59 > BUM',
    );
  ...
  test('# variant 7', () => {
    ...
    message.text =
      'F37AMCLL93#M1BPOS/ID746026,,/DC03032024,173207/MR1,/ET031846/PSN42579W108090,173207,320,WAIDE,031759,WEDAK,M49,267070,T468/CG264,110,360/FB742/VR324E17';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.remaining.text).toBe('F37A#M1B/267070,T468');
  });

  test('variant 8', () => {
    // https://app.airframes.io/messages/2500335076
    message.text =
      'POS/TS080616,210324/DTMMGL,29O,64,103316/PR1754,231,350,189,,0,0,,M45,185,,,P16,P0,36000,,1565,250/RP:DA:MMTJ:AA:MMGL:R:27O:D:TUMA2B..SANFE.UT4..LMM:A:LONV1D:AP:ILSZ29.PLADE(29O)9D1C';
    const decodeResult = plugin.decode(message);
  ...
  test('variant 1 with offset', () => {
    // https://app.airframes.io/messages/2500335076
    message.text =
      'POSN33204W114082,BLH176-0093,062056,330,SALOM180-0127,062211,KOFFA180-0148,M49,30628,3201251';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[2].label).toBe('Aircraft Route');
    expect(decodeResult.formatted.items[2].value).toBe(
      'BLH[176° 9.3nm]@06:20:56 > SALOM[180° 12.7nm]@06:22:11 > KOFFA[180° 14.8nm]',
    );
    expect(decodeResult.formatted.items[3].label).toBe(
      'Outside Air Temperature (C)',
    );
  ...
  test('variant 9', () => {
    ...
    message.text =
      'POSN39164W077259,FORKL,231828,51,THRET,231917,TOOPR,P16,1726,167,/TS231828,2807244841';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[2].label).toBe('Aircraft Route');
    expect(decodeResult.formatted.items[2].value).toBe(
      'FORKL@23:18:28 > THRET@23:19:17 > TOOPR',
    );
    expect(decodeResult.formatted.items[3].label).toBe(
      'Outside Air Temperature (C)',
    );
  ...
  test('variant 10', () => {
    ...
    expect(decodeResult.formatted.items[2].label).toBe('Aircraft Route');
    expect(decodeResult.formatted.items[2].value).toBe(
      'EH556@10:42:41 > CIV@10:52:08 > MEDIL',
    );
    expect(decodeResult.formatted.items[3].label).toBe(
      'Outside Air Temperature (C)',
    );
  ...
  test('decodes duplicate data', () => {
    message.text =
      'POSN39220W078258,MRB18,044034,340,EYT19,044427,COL20,M49,27369,436,813/PSN39220W078258,MRB18,044034,340,EYT19,044427,COL20,M49,27369,436,813,ECON CRZ,0,25140035';
    const decodeResult = plugin.decode(message);
    ...
    expect(decodeResult.formatted.items[2].label).toBe('Aircraft Route');
    expect(decodeResult.formatted.items[2].value).toBe(
      'MRB18@04:40:34 > EYT19@04:44:27 > COL20',
    );
    ...
    expect(decodeResult.formatted.items[7].type).toBe(
      'outside_air_temperature',
    );
    ...
    expect(decodeResult.remaining.text).toBe(
      '27369,436,27369,436,813,ECON CRZ,0,2514',
    );
  });

  test('decodes Label H1 Preamble #M1BPOS <invalid>', () => {
    message.text = '#M1BPOS Bogus message';
    const decodeResult = plugin.decode(message);
