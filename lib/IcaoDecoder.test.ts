...
describe('IcaoDecoder', () => {
  ...
  describe('isMilitary', () => {
    // Germany mil_1(df) explicit tests
    it('should match Germany mil_1(df) range 3ea000-3ebfff', () => {
      expect(new IcaoDecoder('3ea000').isMilitary()).toBe(true);
      expect(new IcaoDecoder('3ebfff').isMilitary()).toBe(true);
      expect(new IcaoDecoder('3eafff').isMilitary()).toBe(true);
    });
    it('should not match just outside Germany mil_1(df) range', () => {
      expect(new IcaoDecoder('3e9fff').isMilitary()).toBe(false); // just before
      expect(new IcaoDecoder('3ec000').isMilitary()).toBe(false); // just after
    });
    // US military
